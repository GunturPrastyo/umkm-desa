@props(['sliders', 'totalUmkm'])

<div id="default-carousel" class="relative w-full max-h-6xl" data-carousel="slide"
    x-data="{ umkm: 0, targetUmkm: {{ $totalUmkm }} }"
    x-init="let u = setInterval(() => {
        if (umkm < targetUmkm) umkm++;
    ...
    <div class="absolute inset-0 z-40 flex flex-col items-center justify-center text-center px-4 sm:px-6"
        ...
        <h1
            ...
            <span class="text-yellow-400">UMKM Desa Tirtomulyo</span>. 
            Dukung produk lokal, belanja mudah langsung maupun online!
        </h1>

        <!-- Counter -->
        <div class="mt-6 sm:mt-8 flex flex-col sm:flex-row gap-6 sm:gap-12 text-white font-bold text-xl sm:text-2xl md:text-3xl">
